...
`),R=!0);const a=this._options.WebSocket||WebSocket;this._debug("connect",{url:i,protocols:o}),this._ws=o?new a(i,o):new a(i),this._ws.binaryType=this._binaryType,this._connectLock=!1,this._addListeners(),this._connectTimeout=setTimeout(()=>this._handleTimeout(),n)}).catch(i=>{this._connectLock=!1,this._handleError(new L.ErrorEvent(Error(i.message),this))})}_handleTimeout(){this._debug("timeout event"),this._handleError(new L.ErrorEvent(Error("TIMEOUT"),this))}_disconnect(s=1e3,n){if(this._clearTimeouts(),!!this._ws){this._removeListeners();try{(this._ws.readyState===this.OPEN||this._ws.readyState===this.CONNECTING)&&this._ws.close(s,n),this._handleClose(new L.CloseEvent(s,n,this))}catch{}}}_acceptOpen(){this._debug("accept open"),this._retryCount=0}_removeListeners(){this._ws&&(this._debug("removeListeners"),this._ws.removeEventListener("open",this._handleOpen),this._ws.removeEventListener("close",this._handleClose),this._ws.removeEventListener("message",this._handleMessage),this._ws.removeEventListener("error",this._handleError),this._ws.addEventListener("error",J))}_addListeners(){this._ws&&(this._debug("addListeners"),this._ws.addEventListener("open",this._handleOpen),this._ws.addEventListener("close",this._handleClose),this._ws.addEventListener("message",this._handleMessage),this._ws.addEventListener("error",this._handleError))}_clearTimeouts(){clearTimeout(this._connectTimeout),clearTimeout(this._uptimeTimeout)}};const X=e=>e[1]!==null&&e[1]!==void 0;function Z(){if(crypto!=null&&crypto.randomUUID)return crypto.randomUUID();let e=Date.now(),t=(performance==null?void 0:performance.now)&&performance.now()*1e3||0;return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(s){let n=Math.random()*16;return e>0?(n=(e+n)%16|0,e=Math.floor(e/16)):(n=(t+n)%16|0,t=Math.floor(t/16)),(s==="x"?n:n&3|8).toString(16)})}function B(e,t,s={}){const{host:n,path:i,protocol:o,room:a,party:d,basePath:h,prefix:p,query:f}=e;let u=n.replace(/^(http|https|ws|wss):\/\//,"");if(u.endsWith("/")&&(u=u.slice(0,-1)),i!=null&&i.startsWith("/"))throw new Error("path must not start with a slash");const x=d??"main",S=i?`/${i}`:"",N=o||(u.startsWith("localhost:")||u.startsWith("127.0.0.1:")||u.startsWith("192.168.")||u.startsWith("10.")||u.startsWith("172.")&&u.split(".")[1]>="16"&&u.split(".")[1]<="31"||u.startsWith("[::ffff:7f00:1]:")?t:`${t}s`),P=`${N}://${u}/${h||`${p||"parties"}/${x}/${a}`}${S}`,I=(D={})=>`${P}?${new URLSearchParams([...Object.entries(s),...Object.entries(D).filter(X)])}`,U=typeof f=="function"?async()=>I(await f()):I(f);return{host:u,path:S,room:a,name:x,protocol:N,partyUrl:P,urlProvider:U}}var ee=class extends z{constructor(t){var n,i;const s=A(t);super(s.urlProvider,s.protocols,s.socketOptions);c(this,"_pk");c(this,"_pkurl");c(this,"name");c(this,"room");c(this,"host");c(this,"path");c(this,"basePath");if(this.partySocketOptions=t,this.setWSProperties(s),!t.startClosed&&!this.room&&!this.basePath)throw this.close(),new Error("Either room or basePath must be provided to connect. Use startClosed: true to create a socket and set them via updateProperties before calling reconnect().");t.disableNameValidation||((n=t.party)!=null&&n.includes("/")&&console.warn(`PartySocket: party name "${t.party}" contains forward slash which may cause routing issues. Consider using a name without forward slashes or set disableNameValidation: true to bypass this warning.`),(i=t.room)!=null&&i.includes("/")&&console.warn(`PartySocket: room name "${t.room}" contains forward slash which may cause routing issues. Consider using a name without forward slashes or set disableNameValidation: true to bypass this warning.`))}updateProperties(t){const s=A({...this.partySocketOptions,...t,host:t.host??this.host,room:t.room??this.room,path:t.path??this.path,basePath:t.basePath??this.basePath});this._url=s.urlProvider,this._protocols=s.protocols,this._options=s.socketOptions,this.setWSProperties(s)}setWSProperties(t){const{_pk:s,_pkurl:n,name:i,room:o,host:a,path:d,basePath:h}=t;this._pk=s,this._pkurl=n,this.name=i,this.room=o,this.host=a,this.path=d,this.basePath=h}reconnect(t,s){if(!this.host)throw new Error("The host must be set before connecting, use `updateProperties` method to set it or pass it to the constructor.");if(!this.room&&!this.basePath)throw new Error("The room (or basePath) must be set before connecting, use `updateProperties` method to set it or pass it to the constructor.");super.reconnect(t,s)}get id(){return this._pk}get roomUrl(){return this._pkurl}static async fetch(t,s){const n=B(t,"http"),i=typeof n.urlProvider=="string"?n.urlProvider:await n.urlProvider();return(t.fetch??fetch)(i,s)}};function A(e){const{id:t,host:s,path:n,party:i,room:o,protocol:a,query:d,protocols:h,...p}=e,f=t||Z(),u=B(e,"ws",{_pk:f});return{_pk:f,_pkurl:u.partyUrl,name:u.name,room:u.room,host:u.host,path:u.path,basePath:e.basePath,protocols:h,socketOptions:p,urlProvider:u.urlProvider}}let m=null,l=null,w=null;const T=["var(--kw-1)","var(--kw-2)","var(--kw-3)","var(--kw-4)"],r=e=>document.getElementById(e),_=e=>{document.querySelectorAll(".screen").forEach(t=>t.classList.remove("active")),r(e).classList.add("active")};function b(e){const t=r("toast");t.textContent=e,t.classList.add("show"),setTimeout(()=>t.classList.remove("show"),2500)}function W(e,t){const s=location.hostname==="localhost"||location.hostname==="127.0.0.1"?"localhost:1999":"decrypto-online.kennyphan123.partykit.dev";m=new ee({host:s,room:e}),m.addEventListener("open",()=>{m.send(JSON.stringify({type:"join",name:t}))}),m.addEventListener("message",n=>{const i=JSON.parse(n.data);i.type==="state"?(l=i.state,se()):i.type==="error"&&b(i.message)}),m.addEventListener("close",()=>{b("Mất kết nối. Tải lại trang để chơi lại.")})}function k(e){m&&m.send(JSON.stringify(e))}r("btn-create").addEventListener("click",()=>{const e=r("player-name").value.trim();if(!e){b("Vui lòng nhập tên");return}const t=te();W(t,e),_("lobby-screen")});r("btn-join").addEventListener("click",()=>{const e=r("player-name").value.trim(),t=r("room-code-input").value.trim().toUpperCase();if(!e){b("Vui lòng nhập tên");return}if(!t||t.length<4){b("Vui lòng nhập mã phòng");return}W(t,e),_("lobby-screen")});r("player-name").addEventListener("keydown",e=>{e.key==="Enter"&&r("btn-create").click()});r("room-code-input").addEventListener("keydown",e=>{e.key==="Enter"&&r("btn-join").click()});function te(){const e="ABCDEFGHJKLMNPQRSTUVWXYZ";let t="";for(let s=0;s<4;s++)t+=e[Math.floor(Math.random()*e.length)];return t}r("btn-copy-code").addEventListener("click",()=>{const e=r("room-code-text").textContent;navigator.clipboard.writeText(e).then(()=>b("Đã sao chép"))});r("btn-start").addEventListener("click",()=>k({type:"start"}));r("history-toggle").addEventListener("click",()=>{r("history-panel").classList.toggle("open")});r("btn-play-again").addEventListener("click",()=>k({type:"play-again"}));r("btn-back-home").addEventListener("click",()=>{m&&m.close(),m=null,l=null,_("home-screen")});function se(){l&&(w&&(clearInterval(w),w=null),l.phase==="LOBBY"?(ie(),_("lobby-screen")):l.phase==="GAME_OVER"?(_e(),_("gameover-screen")):(oe(),_("game-screen"),ne()))}function ne(){const e=l,t=r("topbar-timer");if(e.phase==="ENCRYPT"&&e.timerEnd){t.style.display="block";const s=()=>{const n=Math.max(0,Math.floor((e.timerEnd-Date.now())/1e3));if(t.textContent=n+"s",n<=0){clearInterval(w);const i=r("btn-submit-clues");i&&!i.disabled&&i.click()}};s(),w=setInterval(s,1e3)}else t.style.display="none"}function ie(){var n;const e=l;r("room-code-text").textContent=e.roomCode;const t=(n=e.players.find(i=>i.id===e.myId))==null?void 0:n.isHost,s=e.players.length;s<3?r("lobby-mode-info").textContent=`${s} người chơi — Cần ít nhất 3 người`:s===3?r("lobby-mode-info").textContent="3 người chơi — Chế độ độc lập (Interceptor)":r("lobby-mode-info").textContent=`${s} người chơi — Chế độ đội`,s>=4&&e.teamA&&e.teamB?(r("lobby-players").style.display="none",r("lobby-teams").style.display="flex",M("team-a-list",e.teamA.players,e.myId),M("team-b-list",e.teamB.players,e.myId)):(r("lobby-players").style.display="block",r("lobby-teams").style.display="none",r("lobby-players").innerHTML=e.players.map(i=>`
      <div class="lobby-player-item">
        <div class="lobby-player-dot"></div>
        <span class="lobby-player-name">${y(i.name)}</span>
    ...
    <li>${y(n.name)}${n.id===s?" (bạn)":""}</li>
  `).join("")}function oe(){const e=l;re(),r("round-display").textContent=`${e.round}/${e.maxRounds}`,ae(),ce(),le(),de(),ge()}function re(){const e=l,t=e.players.find(i=>i.id===e.myId);if(!t)return;let s=`<strong>${y(t.name)}</strong>`;e.mode==="3p"?s+=e.myRole==="interceptor"?" (Người chặn)":" (Đội mã hóa)":s+=e.myTeam?` (Đội ${e.myTeam})`:" (Khán giả)";const n=r("player-identity");n&&(n.innerHTML=s)}function ae(){const e=l,t=r("topbar-tokens");e.mode==="3p"?t.innerHTML=`
      <div class="token-3p">
    ...
    `}function ce(){const e=l,t=r("keywords-panel");e.keywords?(t.style.display="block",r("keywords-list").innerHTML=e.keywords.map((s,n)=>`
      <div class="keyword-chip kw-${n+1}">
        <span class="kw-number">${n+1}</span>
        <span>${y(s)}</span>
      </div>
    `).join("")):t.style.display="none"}function le(){var s;const e=l;let t="";if(e.mode==="3p"){const n=((s=e.encryptors.find(i=>i.id===e.currentEncryptorId))==null?void 0:s.name)||"";switch(e.phase){case"ENCRYPT":t=e.myRole==="encryptor"?"Bạn là người mã hóa — Nhập gợi ý":`Đang chờ ${n} mã hóa...`;break;case"GUESS":t="Đoán mã số";break;case"REVEAL":t="Kết quả";break}}else{const n=e.currentTeamTurn;switch(e.phase){case"ENCRYPT":t=e.myRole==="encryptor"?"Bạn là người mã hóa — Nhập gợi ý":"Đang chờ mã hóa...";break;case"GUESS_A":case"GUESS_B":t=`Đội ${n} — Đoán mã số`;break;case"REVEAL_A":case"REVEAL_B":t=`Đội ${n} — Kết quả`;break}}r("phase-status").textContent=t}function de(){const e=l,t=r("action-area");e.phase==="ENCRYPT"?ue(t):e.phase==="GUESS"||e.phase==="GUESS_A"||e.phase==="GUESS_B"?he(t):(e.phase==="REVEAL"||e.phase==="REVEAL_A"||e.phase==="REVEAL_B")&&fe(t)}function ue(e){const t=l,s=t.mode==="3p"?t.cluesSubmitted:t.myTeam?t["team"+t.myTeam].cluesSubmitted:!1;if(t.myRole==="encryptor"&&t.code&&!s)e.innerHTML=`
      <div class="encrypt-code-display fade-in">
        ...
        <div class="encrypt-code-numbers">
          ${t.code.map(n=>`<div class="code-digit" style="background:${T[n-1]}">${n}</div>`).join("")}
        </div>
      </div>
      <div class="clue-inputs fade-in">
        ${t.code.map((n,i)=>`
          <div class="clue-input-row">
            <div class="clue-number" style="background:${T[n-1]}">${["A","B","C"][i]}</div>
    ...
    `,r("btn-submit-clues").addEventListener("click",n=>{const i=!n.isTrusted;let o=[0,1,2].map(a=>r(`clue-${a}`).value.trim());if(!i&&o.some(a=>!a)){b("Vui lòng nhập đủ 3 gợi ý");return}i&&(o=o.map(a=>a||"(Hết giờ)")),k({type:"submit-clues",clues:o}),e.innerHTML=`
    ...
    `}}function he(e){const t=l,s=t.currentClues||t.clues;if(!s)return;let n="";n+=`
      ...
      <div class="guess-inputs">
        ${[0,1,2].map(s=>`
          <div class="guess-select-group">
            ...
            <select class="guess-select" id="guess-${s}">
              <option value="">?</option>
  ...
  `}function ye(){const e=r("btn-submit-guess");e&&e.addEventListener("click",()=>{const t=e.dataset.type,s=[0,1,2].map(n=>{var o;const i=(o=r(`guess-${n}`))==null?void 0:o.value;return i?parseInt(i):0});if(s.some(n=>n<1||n>4)){b("Vui lòng chọn đủ 3 số");return}k({type:"submit-guess",guess:s,guessType:t}),e.disabled=!0,e.textContent="Đã gửi"})}function fe(e){var i,o;const t=l,s=(i=t.players.find(a=>a.id===t.myId))==null?void 0:i.isHost;let n="";if(n+=`
    <div class="reveal-section fade-in">
      <div class="reveal-title">Mã số đúng</div>
      <div class="reveal-code-row">
        ${t.revealCode.map(a=>`<div class="code-digit" style="background:${T[a-1]}">${a}</div>`).join("")}
      </div>
  `,t.decryptGuess){const a=t.decryptCorrect?"result-correct":"result-incorrect",d=t.decryptCorrect?"Giải mã thành công":"Giải mã thất bại";n+=`
      <div class="reveal-result ${a}">
        <span class="result-label">${d}</span>
        <span class="result-codes">${t.decryptGuess.join(" - ")}</span>
      </div>
    `}if(t.interceptGuess){const a=t.interceptCorrect?"result-correct":"result-incorrect",d=t.interceptCorrect?"Chặn mã thành công!":"Chặn mã thất bại";n+=`
      <div class="reveal-result ${a}">
        <span class="result-label">${d}</span>
        <span class="result-codes">${t.interceptGuess.join(" - ")}</span>
      </div>
    `}else t.round<2||t.needIntercept;n+="</div>",s?n+='<button class="btn btn-primary" id="btn-continue">Tiếp tục</button>':n+='<div class="waiting-indicator">Đang chờ chủ phòng tiếp tục<span class="waiting-dots"></span></div>',e.innerHTML=n,(o=r("btn-continue"))==null||o.addEventListener("click",()=>{k({type:"continue"})})}function ge(){const e=l,t=r("history-tables");e.mode==="3p"?be(t):ve(t)}function be(e){const s=l.history||[];if(s.length===0){e.innerHTML='<div class="history-empty">Chưa có lịch sử</div>';return}const n={1:[],2:[],3:[],4:[]},i=[];for(const o of s){i.push(o.round);for(let a=0;a<3;a++){const d=o.code[a];n[d].push({round:o.round,clue:o.clues[a]})}}e.innerHTML=`
    <div class="history-section-label">Theo từ khóa</div>
    ${$(n,i)}
  `}function ve(e){const t=l,s=t.myHistory||[],n=t.opponentHistory||[];if(s.length===0&&n.length===0){e.innerHTML='<div class="history-empty">Chưa có lịch sử</div>';return}let i="";if(n.length>0){const o=t.myTeam==="A"?"B":"A",a={1:[],2:[],3:[],4:[]},d=[];for(const h of n){d.push(h.round);for(let p=0;p<3;p++){const f=h.code[p];a[f].push({round:h.round,clue:h.clues[p]})}}i+=`<div class="history-section-label">Đội ${o} (đối phương)</div>`,i+=$(a,d)}if(s.length>0){const o={1:[],2:[],3:[],4:[]},a=[];for(const d of s){a.push(d.round);for(let h=0;h<3;h++){const p=d.code[h];o[p].push({round:d.round,clue:d.clues[h]})}}i+=`<div class="history-section-label">Đội ${t.myTeam} (đội bạn)</div>`,i+=$(o,a)}e.innerHTML=i}function $(e,t){const s=[...new Set(t)].sort((i,o)=>i-o);let n='<table class="history-table"><thead><tr><th>Từ khóa</th>';for(const i of s)n+=`<th class="kw-col-header">V${i}</th>`;n+="</tr></thead><tbody>";for(let i=1;i<=4;i++){n+=`<tr><td style="color:${T[i-1]}; font-weight:600">#${i}</td>`;for(const o of s){const a=e[i].find(d=>d.round===o);n+=`<td class="kw-cell">${a?y(a.clue):"—"}</td>`}n+="</tr>"}return n+="</tbody></table>",n}function _e(){var s;const e=l,t=(s=e.players.find(n=>n.id===e.myId))==null?void 0:s.isHost;if(e.mode==="3p")e.winner==="interceptor"?r("gameover-title").textContent=e.myRole==="interceptor"?"Bạn đã thắng!":"Người chặn mã đã thắng!":r("gameover-title").textContent=e.myRole==="interceptor"?"Bạn đã thua!":"Đội mã hóa đã thắng!",r("gameover-summary").innerHTML=`
    ...
    `,e.allKeywords&&(r("gameover-keywords").innerHTML=`
      ...
      `)}r("btn-play-again").style.display=t?"block":"none"}function y(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}
